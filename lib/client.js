        ...
        response.on('end', function(){
           var error, result;
            ...
            if(response.statusCode=== 400){
                ...
                // LitecoinCash: RPC errors get wrapped in a 500 error.
                // Pass the inner error back to the client!
                //error = new Error('Connection Rejected : 500 Internal server error');
                try {
                    var innerError = JSON.parse(data);
                    var msg = innerError.error.message;
                    // Trim the "Error: " if present
